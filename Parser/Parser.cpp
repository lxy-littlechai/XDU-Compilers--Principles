...
Token Parser::fetchToken() {
  ...
  try {
    if (token.tokenType == TokenType::ERR_TOKEN) {
      std::string errMsg =
          "Error in tokenType when fetchToken\n" + token.token + "is ERR_TOKEN";
...
void Parser::matchToken(TokenType tokenType) {
  try {
    if (token.tokenType != tokenType) {
      std::string errMsg =
          "Error in tokenType when matchToken\n"
          "now token is " +
          token.token + "\n";
      throw ExceptionLog(scanner.row, scanner.column, errMsg);
    }
...
void Parser::judgeSentenceType() {
  try {
    switch (token.tokenType) {
      ...
      default: {
        throw ExceptionLog(scanner.row, scanner.column,
                           "Error in sentenceType");
      }
    }
  } catch (ExceptionLog& e) {
    e.log();
...
std::shared_ptr<TreeNode> Parser::Term() {
  ...
  left = Factor();
  while (token.tokenType == TokenType::MUL ||
...
// Atom -> CONST_ID | T | FUNC L_BRACKET expression R_BRACKET | L_BRACKET
...
std::shared_ptr<TreeNode> Parser::buildTreeNode(
    TokenType tokenType,
...
std::shared_ptr<TreeNode> Parser::buildTreeNode(TokenType tokenType,
                                                double value) {
  auto node = std::make_shared<TreeNode>();
  node->tokenType = tokenType;
